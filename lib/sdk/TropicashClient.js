...
export class TropicashClient {
  ...
  async ping() {
    ...
    try {
      const resp = await this._fetch(`${baseUrl}/ping`, {
        method: 'GET',
        headers: {
      ...
      if (resp.ok && json && json.ok) {
        return { ok: true, environment: json.environment || this.getEnvironment() };
      }
      return {
        ok: false,
        error: (json && json.error) || 'Ping request failed.',
        status: resp.status,
      };
    } catch (err) {
      return { ok: false, error: err?.message || 'Network error during ping.' };
    }
  }
}
