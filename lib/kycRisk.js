 ...
 * Phase 11J: Funding/send cumulative daily usage (server-enforced as of TLP-002).
...
const FALLBACK_POLICIES = {
  ...
  submitted: { funding_daily_limit: 500, send_daily_limit: 200, withdrawal_daily_limit: 200, enforcement_mode: "soft_block", is_active: true },
  under_review: { funding_daily_limit: 500, send_daily_limit: 200, withdrawal_daily_limit: 200, enforcement_mode: "soft_block", is_active: true },
  rejected: { funding_daily_limit: 100, send_daily_limit: 50, withdrawal_daily_limit: 50, enforcement_mode: "hard_block", is_active: true },
  needs_more_info: { funding_daily_limit: 100, send_daily_limit: 50, withdrawal_daily_limit: 50, enforcement_mode: "hard_block", is_active: true },
  not_started: { funding_daily_limit: 250, send_daily_limit: 100, withdrawal_daily_limit: 100, enforcement_mode: "hard_block", is_active: true },
  missing: { funding_daily_limit: 250, send_daily_limit: 100, withdrawal_daily_limit: 100, enforcement_mode: "hard_block", is_active: true },
};
...
export async function evaluateKycTransactionLimit({ userId, actionType, amount, supabaseClient } = {}) {
  ...
  const advisoryOnly = policyAdvisoryOnly;
  ...
  if (exceedsLimit && !advisoryOnly) {
    ...
    if (action === "withdrawal") {
      ...
      reason =
        mode === "hard_block"
          ? "Daily funding total would exceed your KYC hard daily limit."
          : "Daily funding total would exceed your recommended KYC daily limit.";
    } else {
      reason =
        mode === "hard_block"
          ? "Daily send total would exceed your KYC hard daily limit."
          : "Daily send total would exceed your recommended KYC daily limit.";
    }
  } else if (exceedsLimit && advisoryOnly) {
    if (action === "withdrawal") {
      reason = "Daily withdrawal total exceeds recommended KYC daily limit (advisory only — not blocked).";
    } else if (action === "funding") {
      reason = "Daily funding total exceeds recommended KYC daily limit (advisory only — not blocked).";
    } else {
      reason = "Daily send total exceeds recommended KYC daily limit (advisory only — not blocked).";
    }
  }

  return {
    allowed: advisoryOnly && exceedsLimit ? true : allowed,
    mode,
    reason,
    kycStatus,
    policyStatus,
