...
export async function evaluateKycTransactionLimit({ userId, actionType, amount, supabaseClient } = {}) {
  ...
  if (exceedsLimit && !advisoryOnly) {
    ...
    reason =
      action === "withdrawal"
        ? "Daily withdrawal total exceeds recommended KYC daily limit (advisory only — not blocked yet)."
        : "Amount exceeds recommended KYC daily limit (advisory only — not blocked yet).";
  }
