...
async function resolvePrimaryAdminId(client) {
  ...
  const adminEmail = String(ADMIN_EMAILS[0] || "").trim().toLowerCase();
  if (!adminEmail || !client) return null;
  try {
    ...
    cachedPrimaryAdminId = String(data.id);
...
/**
 * Convenience: route an event to the primary admin (lib/adminAccess ADMIN_EMAILS[0]).
