...
function probePayPalModeParity() {
  const pub = typeof process.env.NEXT_PUBLIC_PAYPAL_MODE === "string"
    ? process.env.NEXT_PUBLIC_PAYPAL_MODE.trim().toLowerCase()
    : "";
  const srv = typeof process.env.PAYPAL_MODE === "string" ? process.env.PAYPAL_MODE.trim().toLowerCase() : "";
  if (!pub && !srv) {
    return {
      status: AUDIT_STATUS.PARTIAL,
      detail: "Neither PAYPAL_MODE nor NEXT_PUBLIC_PAYPAL_MODE set — defaults to sandbox.",
    };
  }
  if (pub && srv && pub !== srv) {
    return {
      status: AUDIT_STATUS.MISSING,
      detail: `PayPal mode mismatch: server=${srv} vs public=${pub}. Funding may fail in production.`,
    };
  }
  return {
    status: AUDIT_STATUS.READY,
    detail: pub && srv ? `PayPal modes aligned (${pub}).` : "Single PayPal mode var set; confirm parity in hosting.",
  };
}
...
export async function fetchProductionAudit({ supabase } = {}) {
  ...
  const paypalModeParity = probePayPalModeParity();
  ...
  const sections = [
    ...
    },
    {
      ...
      items: [
        ...
        auditItem({
          id: "paypal_mode_parity",
          label: "PayPal server/public mode parity",
          status: paypalModeParity.status,
          notes: "PAYPAL_MODE must match NEXT_PUBLIC_PAYPAL_MODE.",
          recommendedAction:
            paypalModeParity.status === AUDIT_STATUS.READY
              ? "Re-verify after deploy env changes."
              : "Align PAYPAL_MODE and NEXT_PUBLIC_PAYPAL_MODE in hosting dashboard.",
          detail: paypalModeParity.detail,
        }),
      ],
    },
    {
