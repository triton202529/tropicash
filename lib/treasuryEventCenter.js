...
import { buildKycRiskProfileFromStatus, fetchKycStatusMapForUsers, fetchKycLimitPolicies, fetchWithdrawalDailyUsageMapForUsers, summarizeKycLimitPolicy, evaluateWithdrawalEnforcementFromPolicy } from "./kycRisk";
...
async function fetchWithdrawalEvents(supabase, limit) {
  try {
    ...
    const [kycStatusMap, policiesResult, usageByUserId] = await Promise.all([
      fetchKycStatusMapForUsers(userIds),
      fetchKycLimitPolicies(),
      fetchWithdrawalDailyUsageMapForUsers(userIds, { supabaseClient: supabase }),
    ]);
    ...
    return rows
      .map((row) => {
        ...
        const usedToday = Number(usageByUserId[row.user_id]) || 0;
        ...
        normalized.metadata = safeMetadata({
          ...
          withdrawalDailyUsedToday: usedToday,
          withdrawalDailyRemaining: enforcement.remainingToday,
          withdrawalProjectedTotal: usedToday,
