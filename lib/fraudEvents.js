 ...
 * }} p
 */
export async function logFraudStatusChanged(supabase, p) {
  const eventType = statusToEventType(p.nextStatus);
  const result = await createFraudEvent(supabase, {
  ...
  await appendAuditEvent({
    ...
    entityId: p.fraudLogId,
    eventType: "fraud.status_changed",
    actorUserId: p.actorUserId,
    targetUserId: p.userId ?? null,
    severity: "info",
    title: "Fraud review status changed",
    description: `Status ${String(p.previousStatus || "").toLowerCase()} → ${String(p.nextStatus || "").toLowerCase()}.`,
    metadata: {
      previous_status: String(p.previousStatus || "").toLowerCase(),
      next_status: String(p.nextStatus || "").toLowerCase(),
    },
    dedupeKey: `audit:fraud_case:${p.fraudLogId}:status:${p.nextStatus}:${p.reviewedAt || ""}`.slice(0, 380),
