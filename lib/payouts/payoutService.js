 ...
 * @param {{ userId: string; amount: number; outcome: 'processing' | 'paid' | 'failed' }} params
 */
async function notifyWithdrawalPayoutOutcome(supabaseAdmin, params) {
  const { userId, amount, outcome } = params;
  ...
  const cfg =
    outcome === "processing"
      ? {
          ...
          p_message: "Your withdrawal payout is processing.",
        }
      : outcome === "paid"
        ? {
        ...
        : outcome === "failed"
          ? {
              ...
              p_message: "Your withdrawal payout failed. Support will review it.",
            }
          : null;
  if (!cfg) return;
...
export async function applyPayPalTransition(supabaseAdmin, row, delta) {
  ...
  if (statusChanged) {
    if (nextStatus === "paid") {
      await notifyWithdrawalPayoutOutcome(supabaseAdmin, { userId, amount, outcome: "paid" });
    } else if (nextStatus === "failed") {
      await notifyWithdrawalPayoutOutcome(supabaseAdmin, { userId, amount, outcome: "failed" });
    } else if (nextStatus === "processing") {
      const alreadyNotified = row.payout_processing_notified_at != null;
      if (!alreadyNotified) {
        await notifyWithdrawalPayoutOutcome(supabaseAdmin, { userId, amount, outcome: "processing" });
        await supabaseAdmin
