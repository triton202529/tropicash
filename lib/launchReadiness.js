...
async function fetchWithdrawalMetrics(supabase) {
  ...
  const [pending, processing, paidToday, kycBlocked, overLimitAttempts, reconciliation] = await Promise.all([
  ...
  return {
    ...
    reconciliationCritical: reconciliation?.summary?.critical ?? null,
    reconciliationWarnings: reconciliation?.summary?.warning ?? null,
    reconciliationTotal: reconciliation?.summary?.total ?? null,
    error:
      pending.error ||
      processing.error ||
      paidToday.error ||
...
function computeCategoryScores({ checklist, audit, kyc, withdrawals, treasurySummary, riskCases, security, fraud }) {
  ...
  if ((withdrawals?.reconciliationCritical ?? 0) > 0) {
    treasuryScore = clampScore(treasuryScore - Math.min(withdrawals.reconciliationCritical * 8, 24));
  }
  ...
  return {
    ...
    sections: {
      ...
      withdrawals: {
        ...
        reconciliationHref: "/admin/withdrawal-reconciliation",
      },
      treasury: {
        title: "Treasury",
        metrics: {
