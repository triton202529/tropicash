  ...
  try {
    ...
    if (error) {
      ...
      void logOperationalError({
        ...
        category: "fraud.phase1_insert",
        message: error.message || "fraud_logs insert failed",
      ...
      void recordEventOnce({
        supabaseClient: supabase,
        adminTarget: true,
        eventType: "fraud.insert_failed",
        category: "fraud",
        severity: "warning",
        title: "Fraud log insert failed",
        message: `fraud_logs insert failed for event ${payload.eventType}.`,
        metadata: { eventType: payload.eventType, transactionType: payload.transactionType, userId: payload.userId },
        dedupeKey: `fraud.insert_failed:${payload.userId}:${payload.eventType}:${payload.transactionType}`,
        windowMs: 60 * 1000,
      });
    }
  } catch (e) {
    ...
    void logOperationalError({
      ...
      userId: payload.userId,
      route: null,
      metadata: { eventType: payload.eventType, threw: true },
    });
    void recordEventOnce({
      supabaseClient: supabase,
      adminTarget: true,
      ...
      severity: "warning",
      title: "Fraud log insert failed",
      message: `fraud_logs insert threw for event ${payload.eventType}.`,
      metadata: { eventType: payload.eventType, transactionType: payload.transactionType, userId: payload.userId },
      dedupeKey: `fraud.insert_failed:${payload.userId}:${payload.eventType}:${payload.transactionType}`,
      windowMs: 60 * 1000,
    });
